...
function initTimeSheetForm() {
    ...
    // TODO посмотреть можно ли уйти от таймера
    // Во время вызова initTimeSheetForm() менялись значения комбиков -> появлялись события change -> из-за change форма менялась повторно/сбрасывались ошибки
    // Обработка событий должна появляеться после того, как страница будет загружена и значения комбиков установлены.
    setTimeout(function() {
